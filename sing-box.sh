...
C[61]="没有可更换的Argo 隧道"
E[62]="Add / Remove protocols (nb -r)"
C[62]="增加 / 删除协议 (nb -r)"
E[63]="Close Realm"
C[63]="关闭 Realm"
...
E[92]="Change the Argo tunnel (nb -t)"
C[92]="更换 Argo 隧道 (nb -t)"
...
E[115]="Quick install mode (all protocols + subscription) (nb -k)"
C[115]="极速安装模式 (所有协议 + 订阅) (nb -l)"
...
E[121]="Change node configuration (nb -d)"
C[121]="修改节点配置 (nb -d)"
...
fetch_nodes_value() {
  ...
  # 提取不到时，仅在安装相关流程走交互式输入；其余只读流程（如 nb -n 查看节点信息）用随机 UUID 兜底，避免无谓弹窗
...
menu_setting() {
  if [[ "${STATUS[0]}" =~ $(text 27)|$(text 28) ]]; then
    OPTION[1]="1 .  $(text 29)"
    [ "${STATUS[0]}" = "$(text 28)" ] && OPTION[2]="2 .  $(text 27) Sing-box (nb -s)" || OPTION[2]="2 .  $(text 28) Sing-box (nb -s)"
    [ "${STATUS[1]}" = "$(text 28)" ] && OPTION[3]="3 .  $(text 27) Argo (nb -a)" || OPTION[3]="3 .  $(text 28) Argo (nb -a)"
    OPTION[4]="4 .  $(text 92)"
    OPTION[5]="5 .  $(text 121)"
    OPTION[6]="6 .  $(text 31)"
    OPTION[7]="7 .  $(text 32)"
